...
# ══ 2 MANDATE ══
s=base('What we were asked to deliver','Scope')
...
# ══ 4 PIPELINE ══
s=base('From 837 columns to 330 analysable fields','Step 1 — Load and inspect')
...
# ══ 5 NON-INVESTMENT ══
s=base('One column, three different kinds of thing','Step 2 — Classify Source Asset')
...
# ══ 6 HEADLINES ══
s=base('Headline results','Findings')
...
# ══ 7 FIELD BY CATEGORY ══
chart_slide('Completeness is not uniform across field families','Step 3 — Score by field',
...
# ══ 8 RETURNS CLIFF ══
s=base('The returns cliff','Finding 1')
...
# ══ 9 DEAL STATUS ══
chart_slide('Deal Status — the field the brief singles out','Finding 2',
...
s=base('Systematic gaps versus scattered gaps','Step 3 — Field-level distribution')
...
chart_slide('Fund and manager completeness distributions','Step 4 — Score by fund',
...
# ══ 12 FUND BANDS ══
s=base('Where the funds sit','Step 4 — Score by fund')
...
# ══ 13 OUTLIERS ══
chart_slide('The distribution is one-tailed','Step 4 — Outlier detection',
 D+'outliers.png',
...
# ══ 14 SIZE ══
s=base('Larger funds report better — but size is not the mechanism','Step 4 — Size versus quality')
...
s=base('Manager scorecard','Step 5 — Score by manager')
...
# ══ 16 HEATMAP ══
chart_slide('Which field does each weak manager omit?','Step 5 — Manager x field',
...
# ══ 17 SPREAD ══
chart_slide('The most actionable cut in the analysis','Step 5 — Within-manager variance',
...
# ══ 18 INVESTMENT TYPE ══
chart_slide('Quality by investment type','Step 3 — Cross-cut',
...
# ══ 19 REAL ASSETS ══
s=base('Real assets fail an equity-shaped test','Finding 3')
...
# ══ 20 CONCENTRATION ══
chart_slide('Portfolio concentration','Step 6 — Cross-cut',
...
# ══ 21 HYGIENE ══
chart_slide('The two most-reported "companies" were journal entries','Finding 4 — resolved',
...
# ══ 22 GEOGRAPHY ══
chart_slide('Geographic distribution','Step 6 — Cross-cut',
 D+'geography.png',
...
s=base('Entity resolution — sizing the Phase 2 problem','Step 6')
...
# ══ 24 FINDINGS MATRIX ══
s=base('Findings ranked','Synthesis')
...
# ══ 26 PHASE 2 ══
s=base('Phase 1 closes, Phase 2 opens','Roadmap')
...
# ══ 27 APPENDIX ══
s=base('Exports and validation','Appendix')
